(function () {
        ...
        _updateSnap: function (marker, layer, latlng) {
            if (layer && latlng) {
                ...
                if (marker.snap != layer) {
                    ...
                    // Patch to use with Leaflet.Draw. With Leaflet.Draw, you cannot add a listener to the
                    // shape being drawn, so fire the event on map
                    marker._map.fire('snap', {layer:layer, latlng: latlng});
    ...
    L.Draw.Feature.SnapMixin = {
        _snap_initialize: function () {
            ...
            */
            // Patch to work with latest Leaflet.Draw version
            // See https://github.com/makinacorpus/Leaflet.Snap/issues/11#issuecomment-36771398
            var _this = this;
