			...
			.controller(
					...
					[
							...
							function($attrs, $scope, $rootScope, GeoxmlService,
								...
								//$log.debug("    >>>> Using HbCommandeCardController, reallocate = " + $scope.reallocate );
								...
								$scope.displayBuildingAddress = function(noSai,owner) {
									...
									if (noSai !== null && owner !== null && owner !== undefined) {
										...
										//$log.debug(">>>> Using HbCommandeCardController - displayBuildingAddress: NULL parameter value !)");
...
//
								...
								$scope.getPrestationPerId = function(ID_G, Id) {
									$log.debug(">>>>>> HbCommandeCardController - $scope.getPrestationPerId : ID_G / Id = " + ID_G + " / " + Id);
									GeoxmlService.getElfin(ID_G,Id).get().then(
											function(prestation) {
												...
												$log.debug(">>>>>> HbCommandeCardController - $scope.constatPrestation = " + $scope.constatPrestation);												
											},
											function(response) {
								...
								$scope.getPrestation = function(sai,groupePrestation,year,owner) {
									...
									if ( sai !== null && groupePrestation !== null && groupePrestation.length > 0 && year !== null && owner !== undefined && owner !== null && owner.Id !== null && owner.Id !== 'null' && owner.Id.length > 0) {
										...
										$log.debug(">>>>>> HbCommandeCardController - getPrestation = " + "sai = " + sai +", groupePrestation = " + groupePrestation + ", year = " + year + ", owner = " + angular.toJson(owner));
						    	...
						    	$scope.$watch('elfin.GROUPE', function() {
						    		if ($scope.elfin!==null ) {
						    			if ($scope.elfin.GROUPE!==null) {
							    			...
							    			$log.debug(">>>>>> HbCommandeCardController - ($watch('elfin.GROUPE') updating $scope.elfin.CARACTERISTIQUE.CAR1.UNITE (PrestationGroupForTransactionGroup) from >" + $scope.elfin.CARACTERISTIQUE.CAR1.UNITE + "< to >" + prestaGrp +"<");
