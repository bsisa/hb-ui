					...
					function($attrs, $scope, GeoxmlService, $modal, $routeParams,
				    	...
				    	$scope.$watchCollection('[elfin.IDENTIFIANT.OBJECTIF,elfin.IDENTIFIANT.ORIGINE]', function(newValues, oldValues) {							    		
				    		...
				    		if ($scope.elfin!=null && $attrs.hbMode != "create") {
								...
								xpathForImmeubleLinkedToCurrentConstat = "//ELFIN[IDENTIFIANT/OBJECTIF='"+saiNb+"' and IDENTIFIANT/NOM='"+buildingNb+"']";
								hbQueryService.getImmeubles(xpathForImmeubleLinkedToCurrentConstat)
									.then(function(immeubles) {
										...
										if (immeubles.length === 1) {
											...
											var message = "L'IMMEUBLE correspondant à la PRODUCTION_CHALEUR courrante pour le no de construction "+ buildingNb +" et no SAI "+ saiNb +" n'a pas pu être trouvé dans la base de donnée!";
								            hbAlertMessages.addAlert("danger",message);
										} else {
											var message = "Plusieurs IMMEUBLE correspondent à la PRODUCTION_CHALEUR courrante pour le no de construction: " + buildingNb +" et no SAI "+ saiNb +". Le résultat devrait être unique.";
								            hbAlertMessages.addAlert("danger",message);
										}
									},
									function(response) {
										var message = "Le chargement de l'IMMEUBLE lié à la PRODUCTION_CHALEUR courrante pour le no de construction "+ buildingNb +" et no SAI "+ saiNb + " a échoué (statut de retour: "+ response.status+ ")";
							            hbAlertMessages.addAlert("danger",message);
									});										
