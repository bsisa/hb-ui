    ...
    angular.module('hb5').service('hbUtil', ['$log', '$window', '$filter', 'HB_API', 'HB_ORDER_LINE_TYPE', 'userDetails', function ($log, $window, $filter, HB_API, HB_ORDER_LINE_TYPE, userDetails) {
        ...
        var getCARByPos = function (elfin, pos) {
            if (!!elfin && !!elfin.CARACTERISTIQUE && !!elfin.CARACTERISTIQUE.CARSET && !!elfin.CARACTERISTIQUE.CARSET.CAR) {
                var currentCAR = _.find(elfin.CARACTERISTIQUE.CARSET.CAR, function (CAR) {
                    return CAR.POS === pos;
                });
                return currentCAR;
            } else {
        ...
        var getCByPos = function (Cs, CPos) {
            var searchedC = _.find(Cs, function (C) {
                return C.POS === CPos;
            });
            if (searchedC) {
                return searchedC;
        ...
        var getFractionLCByPos = function (elfin, LPos, CPos) {
            var currentL = _.find(elfin.CARACTERISTIQUE.FRACTION.L, function (L) {
                return L.POS === LPos;
            });
            if (currentL) {
...
//						getLine(HB_ORDER_LINE_TYPE.APPLIED_AMOUNT, "Panneau de chantier", "", "", "0.00", "false" ),
        ...
        var renumberPos = function (array) {
            ...
            var haveAllPos = _.reduce(array, function (memo, element) {
                return (memo && (!_.isUndefined(element.POS)));
            }, true);
            if (!haveAllPos) {
                return;
            } else {
        ...
        var listInvalidFieldsAndErrors = function (form) {
            var errors = "";
            if (form.$invalid && form.$error && form.$error.required) {
                ["required",
                    "email",
                    "max",
                    "maxlength",
                    ...
                    "time",
                    "week",
                    "month"].forEach(function (errorType) {
                    errors += errorType + ": [";
        ...
        return {
            ...
            listInvalidFieldsAndErrors: listInvalidFieldsAndErrors
        };
    }]);
