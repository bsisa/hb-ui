	...
	angular.module('hb5').controller(
			...
			[
					...
					function($attrs, $scope, GeoxmlService, $modal, $routeParams,
						...
						// Used to provide navigation link back to IMMEUBLE link to this CITERNE
						$scope.immeubleRef = null;			 			
			 			
			        	// Navigate to IMMEUBLE linked to the current CITERNE
				    	...
				    	$scope.$watchCollection('[elfin.IDENTIFIANT.OBJECTIF,elfin.IDENTIFIANT.ORIGINE]', function(newValues, oldValues) {							    		
				    		...
				    		if ($scope.elfin!=null && $attrs.hbMode != "create") {
								...
								xpathForImmeubleLinkedToCurrentCiterne = "//ELFIN[IDENTIFIANT/OBJECTIF='"+saiNb+"' and IDENTIFIANT/NOM='"+buildingNb+"']";
								hbQueryService.getImmeubles(xpathForImmeubleLinkedToCurrentCiterne)
									.then(function(immeubles) {
										...
										if (immeubles.length === 1) {
											...
											var message = "L'IMMEUBLE correspondant à la CITERNE courrante pour le no de construction "+ buildingNb +" et no SAI "+ saiNb +" n'a pas pu être trouvé dans la base de donnée!";
								            hbAlertMessages.addAlert("danger",message);
										} else {
											var message = "Plusieurs IMMEUBLE correspondent à la CITERNE courrante pour le no de construction: " + buildingNb +" et no SAI "+ saiNb +". Le résultat devrait être unique.";
								            hbAlertMessages.addAlert("danger",message);
										}
									},
									function(response) {
										var message = "Le chargement de l'IMMEUBLE lié à la CITERNE courrante pour le no de construction "+ buildingNb +" et no SAI "+ saiNb + " a échoué (statut de retour: "+ response.status+ ")";
							            hbAlertMessages.addAlert("danger",message);
									});										
				    	...
				    	$scope.$watch('elfin.Id', function() { 
				    		...
				    		if ($scope.elfin!=null) {
					            ...
					            // Asychronous CITERNE template preloading
					            GeoxmlService.getNewElfin("CITERNE").get()
